...
- [x] **1.1 (H1)** Unlock requires the final segment.
...
- [x] **1.2 (H4/H5)** Reset semantics.
      ...
      `'reset'` on rejection.
- [x] **1.3 (M6)** Corner-cutting.
      Red: from `(50,0)` jump to `(100,50)` with tolerance 24 → expect no advance.
      Green: add `projection.distance <= tolerance` conjunct.
- [x] **1.4 (H2)** Generator self-overlap.
      Red: scan seeded tracks for non-adjacent cross/overlap/touch → expect zero.
      Green: adjacency-aware intersection test.
- [x] **1.5 (M4)** `regeneratePath()` destroys the superseded machine.
- [x] **1.6 (M5)** No `AudioContext` unless sound is enabled.
- [x] **1.7** Reconcile existing core tests with corrected behaviour.

## Phase 2 — Adapter Parity (`react-adapter`, `vue-adapter`, `svelte-adapter`)

- [x] **2.1 (H3)** React hook render stability. Red: inline generator + one parent update →
      bounded renders. Green: structural memo signature.
- [x] **2.2 (M7)** React accepts `bounds`/`track`; Vue and Svelte accept
      `initialState`/`initialProgress`; `width`/`height` retained as aliases.
- [x] **2.3 (M8)** React progress overlay path + `style.css` + `./style.css` export.
- [x] **2.4 (L3)** Type Svelte `children` as `Snippet`; remove `any` from Vue `index.ts`.

## Phase 3 — Release & Versioning (`release-pipeline`)

- [x] **3.1 (M2)** Version tests assert export against own `package.json`.
- [x] **3.2 (M3)** Pin `@heelslide/core` to an exact version in all three adapters.

## Phase 4 — Tooling & Gates (`ci-pipeline`)

- [x] **4.1 (L1)** Remove skip-on-missing guards; make `format:check` able to fail.
- [x] **4.2 (L3)** Enable `no-explicit-any` and the react plugin in `.oxlintrc.json`.
- [x] **4.3 (L4)** Add per-file coverage thresholds.
- [x] **4.4 (L6)** Pass workflow outputs via `env:`.
- [x] **4.5 (L2)** Reconcile `tech-stack.md` with the installed toolchain.

## Phase 5 — Cooper Hygiene

- [x] **5.1 (L5)** Promote the `release-pipeline` living spec; archive merged tracks;
      reconcile `tracks.md`; register `accessibility-fallback`; tear down stale worktrees.
- [x] **5.2** Merge this track's spec deltas into `.cooper/specs/`.
