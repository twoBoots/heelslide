...
- [ ] Task 1.2: Checkpoint Clamping, Resumption & Snapback (`packages/core/src/machine.ts`)
  ...
  - [ ] Sub-task: Implement `end()` transition to `'checkpoint'` and callback dispatch (Green)
  - [ ] Sub-task: Write unit tests for re-engaging handle at checkpoint vertex via `start()` and proceeding to next segment (Red)
  - [ ] Sub-task: Implement checkpoint engagement in `start()` (Green)
  - [ ] Sub-task: Write unit tests for mid-segment release snapping back to last reached checkpoint (Red)
  - [ ] Sub-task: Implement mid-segment snapback in `end()` and `cancel()` (Green)
  - [ ] Sub-task: Refactor & verify core state machine test coverage >80% (Refactor)
...
- [ ] Task 2.2: Vue Adapter Integration (`packages/vue`)
  - [ ] Sub-task: Write component tests for `segmented` prop and `checkpoint` emit in `packages/vue/tests/` (Red)
  - [ ] Sub-task: Implement props, emits, and state synchronization in Vue `<Heelslide />` component (Green)
  - [ ] Sub-task: Refactor & verify Vue adapter test coverage >80% (Refactor)
