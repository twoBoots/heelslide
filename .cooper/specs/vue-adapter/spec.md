...
- **GIVEN** the SVG handle group, which carried `role="slider"`, `tabindex="0"`, and the `aria-value*` attributes prior to this capability
- **WHEN** the component is rendered
- **THEN** the handle MUST be presentational, carrying `role="presentation"`, no `tabindex`, and no `aria-value*` attributes, so assistive technology sees a single slider node.
...
- **GIVEN** a component rendered with `disabled`
- **WHEN** it is rendered
- **THEN** it MUST expose `aria-disabled`, reaching parity with the React and Svelte adapters.
...
- **WHEN** it is rendered
- **THEN** `aria-keyshortcuts` MUST enumerate the bound keys and `aria-describedby` MUST reference a node containing the accessible path description.
...
- **GIVEN** `style.css` sets `outline: none` on `.heelslide-handle`, which prior to this capability was the focusable element
- **WHEN** a keyboard user focuses the component
- **THEN** the indicator MUST be visible.
...
- **WHEN** it is rendered
- **THEN** it MUST render a visually hidden node carrying `role="status"`, `aria-live="polite"`, and `aria-atomic="true"`, overridable via an `#announcer` slot.
- **GIVEN** a consumer listening for the `announcement` event
- **WHEN** the engine emits an announcement
- **THEN** the event MUST be emitted with the `AccessibleAnnouncement` payload.
...
- **THEN** it MUST expose `stepForward`, `stepBackward`, `stepToNextHeel`, and reactive `steps`, `description` and `announcement`.
